import { useState, useEffect } from 'react';
import styles from './GoogleAccountStatus.module.css';

export default function GoogleAccountStatus({ onConnect }) {
  const [status, setStatus] = useState('loading');
  const [accountInfo, setAccountInfo] = useState(null);
  const [disconnecting, setDisconnecting] = useState(false);

  useEffect(() => {
    checkGoogleConnection();
  }, []);

  const checkGoogleConnection = async () => {
    try {
      const response = await fetch('/api/user/connected-accounts');
      const data = await response.json();
      
      const googleAccount = data.accounts?.find(acc => acc.provider === 'google');
      
      if (googleAccount?.isConnected) {
        setStatus('connected');
        setAccountInfo(googleAccount);
      } else {
        setStatus('disconnected');
      }
    } catch (error) {
      console.error('Failed to check Google connection:', error);
      setStatus('error');
    }
  };

  const handleDisconnect = async () => {
    if (!confirm('Are you sure you want to disconnect your Google account? You will no longer be able to schedule Google Meet interviews until you reconnect.')) {
      return;
    }

    setDisconnecting(true);
    try {
      const response = await fetch('/api/user/connected-accounts', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ provider: 'google' }),
      });

      if (response.ok) {
        setStatus('disconnected');
        setAccountInfo(null);
      } else {
        alert('Failed to disconnect Google account');
      }
    } catch (error) {
      console.error('Failed to disconnect:', error);
      alert('Failed to disconnect Google account');
    } finally {
      setDisconnecting(false);
    }
  };

  if (status === 'loading') {
    return (
      <div className={styles.statusLoading}>
        <div className={styles.spinner}></div>
        <span>Checking connection...</span>
      </div>
    );
  }

  if (status === 'connected') {
    return (
      <div className={styles.statusConnected}>
        <div className={styles.statusBadge}>
          <svg className={styles.checkIcon} viewBox="0 0 20 20" fill="currentColor">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
          <span className={styles.statusText}>Connected</span>
        </div>
        <button
          onClick={handleDisconnect}
          disabled={disconnecting}
          className={styles.disconnectButton}
        >
          {disconnecting ? 'Disconnecting...' : 'Disconnect'}
        </button>
      </div>
    );
  }

  return (
    <button onClick={onConnect} className={styles.connectButton}>
      <svg className={styles.googleIcon} viewBox="0 0 24 24">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
      </svg>
      Connect Google Account
    </button>
  );
}
